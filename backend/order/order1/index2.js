...
app.post('/purchase/:id', async (req, res) => {
  ...
  try {
    console.log(`[${sourceReplica}] 🔎 Trying to contact catalog with: http://catalog1:3000/info/${id}`);

    const catalogResponse = await axios.get(`http://catalog1:3000/info/${id}`);
    const { quantity, title } = catalogResponse.data;

    if (quantity > 0) {
      console.log(`✅ [${sourceReplica}] bought book: ${title}`);

      await axios.put(`http://catalog1:3000/update/${id}`, {
        quantity: quantity - 1,
